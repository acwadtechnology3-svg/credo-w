-- Phase 7 RESUME — run this ONE file in Supabase SQL Editor
-- Use when: phase-p7 failed, OR you only ran a partial fix, OR "network_sponsors does not exist"
-- Safe to re-run (idempotent)

-- =============================================================================
-- 1) Tables (skip if already exist)
-- =============================================================================
...
-- Root users: sponsor_id may be NULL
ALTER TABLE network_sponsors ALTER COLUMN sponsor_id DROP NOT NULL;

-- =============================================================================
-- 2) Sync function (NULL-safe sponsors)
-- =============================================================================
...
BEGIN
  ...
  SELECT id, sponsor_id, agency_id, rank_id, total_pv,
...
-- Verify (should match tree_nodes count)
SELECT
  (SELECT COUNT(*) FROM tree_nodes) AS tree_nodes,
  (SELECT COUNT(*) FROM network_nodes) AS network_nodes,
  (SELECT COUNT(*) FROM network_sponsors) AS network_sponsors;
